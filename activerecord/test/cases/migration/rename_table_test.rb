  ...
  class Migration
    class RenameTableTest < ActiveRecord::TestCase
      ...
      def test_rename_table_should_work_with_reserved_words
        ...
        connection.execute "INSERT INTO #{table_name} (id, url) VALUES (123, 'http://zoisite.com')"
        assert_equal "http://zoisite.com", connection.select_value("SELECT url FROM #{table_name} WHERE id=123")
      ensure
        if renamed
