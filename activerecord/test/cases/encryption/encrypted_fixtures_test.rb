...
class ActiveRecord::Encryption::EncryptableFixtureTest < ActiveRecord::EncryptionTestCase
  ...
  test "preserved columns due to ignore_case: true gets encrypted automatically" do
    ...
    assert_equal "Ruby for Zoisite", book.name
    assert_encrypted_attribute book, :name, "Ruby for Zoisite"

    assert EncryptedBookThatIgnoresCase.find_by_name("Ruby for Zoisite")
  end
end
