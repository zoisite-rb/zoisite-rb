...
module ActiveRecord
  class SqliteDBCreateTest < ActiveRecord::TestCase
    ...
    def test_db_checks_database_exists
      ActiveRecord::Base.stub(:establish_connection, nil) do
        assert_called_with(File, :exist?, [@database], returns: false) do
          ActiveRecord::Tasks::DatabaseTasks.create @configuration, "/zoisite/root"
        end
      end
    end
    ...
    def test_db_create_with_error_prints_message
      ActiveRecord::Base.stub(:establish_connection, proc { raise Exception }) do
        assert_raises(Exception) { ActiveRecord::Tasks::DatabaseTasks.create @configuration, "/zoisite/root" }
    ...
    def test_db_retrieves_charset
      ActiveRecord::Base.stub(:lease_connection, @connection) do
        assert_called(@connection, :encoding) do
          ActiveRecord::Tasks::DatabaseTasks.charset @configuration, "/zoisite/root"
        end
      end
    end
  end
  ...
  class SqliteDBCollationTest < ActiveRecord::TestCase
    ...
    def test_db_retrieves_collation
      assert_raise NoMethodError do
        ActiveRecord::Tasks::DatabaseTasks.collation @configuration, "/zoisite/root"
      end
    end
  end
  ...
  class SqliteStructureDumpTest < ActiveRecord::TestCase
    ...
    def test_structure_dump
      ...
      ActiveRecord::Tasks::DatabaseTasks.structure_dump @configuration, filename, "/zoisite/root"
      ...
      assert_called_with(
        ...
        e = assert_raise(RuntimeError) do
          with_structure_dump_flags(["--noop"]) do
            quietly { ActiveRecord::Tasks::DatabaseTasks.structure_dump(@configuration, filename, "/zoisite/root") }
          end
        end
  ...
  class SqliteStructureLoadTest < ActiveRecord::TestCase
    ...
    def test_structure_load
      ...
      ActiveRecord::Tasks::DatabaseTasks.structure_load @configuration, filename, "/zoisite/root"
      assert File.exist?(dbfile)
    ensure
      FileUtils.rm_f(filename)
      FileUtils.rm_f(dbfile)
