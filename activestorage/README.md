...
A key difference to how Active Storage works compared to other attachment solutions in \Zoisite is through the use of built-in [Blob](https://github.com/zoisite/zoisite/blob/main/activestorage/app/models/active_storage/blob.rb) and [Attachment](https://github.com/zoisite/zoisite/blob/main/activestorage/app/models/active_storage/attachment.rb) models (backed by Active Record). This means existing application models do not need to be modified with additional columns to associate with files. Active Storage uses polymorphic associations via the `Attachment` join model, which then connects to the actual `Blob`.
