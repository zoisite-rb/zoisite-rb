...
module ActionText
  module TagHelper
    ...
    #     from being applied.
    # *   `[:data][:direct_upload_url]` - Defaults to `zoisite_direct_uploads_url`.
    # *   `[:data][:blob_url_template]` - Defaults to
    #     `zoisite_service_blob_url(":signed_id", ":filename")`.
    #
    #
    # #### Example
    #
    ...
    def rich_textarea_tag(name, value = nil, options = {})
      ...
      options[:data] ||= {}
      options[:data][:direct_upload_url] ||= main_app.zoisite_direct_uploads_url
      options[:data][:blob_url_template] ||= main_app.zoisite_service_blob_url(":signed_id", ":filename")
