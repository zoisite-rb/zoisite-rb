...
class Zoisite::InfoController < Zoisite::ApplicationController # :nodoc:
  ...
  def routes
    if query = params[:query]
      ...
      @routes_inspector = ActionDispatch::Routing::RoutesInspector.new(Zoisite.application.routes.routes)
      @page_title = "Routes"
    end
  end

  def notes
    tags = params[:tag].presence || Zoisite::SourceAnnotationExtractor::Annotation.tags.join("|")
    @annotations = Zoisite::SourceAnnotationExtractor.new(tags).find(
  ...
  private
    def matching_routes(query:, exact_match:)
      ...
      Zoisite.application.routes.routes.filter_map do |route|
