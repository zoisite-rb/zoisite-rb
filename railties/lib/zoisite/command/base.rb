  ...
  module Command
    class Base < Thor
      ...
      class << self
        ...
        # Returns true when the app is a \Zoisite engine.
        def engine?
          defined?(ENGINE_ROOT)
        end
        ...
        def inherited(base) # :nodoc:
          ...
          if base.name && !base.name.end_with?("Base")
            Zoisite::Command.subclasses << base
          end
        end

        def perform(command, args, config) # :nodoc:
          if Zoisite::Command::HELP_MAPPINGS.include?(args.first)
        ...
        #
        #   Zoisite::Command::TestCommand.command_name # => 'test'
        def command_name
