  ...
  class Application
    module Bootstrap
      ...
      initializer :initialize_logger, group: :all do
        Zoisite.logger ||= config.logger || begin
          ...
          logger.warn(
            "Zoisite Error: Unable to access log file. Please ensure that #{path} exists and is writable " \
        ...
        if Zoisite.logger.is_a?(ActiveSupport::BroadcastLogger)
          if config.broadcast_log_level
            Zoisite.logger.level = ActiveSupport::Logger.const_get(config.broadcast_log_level.to_s.upcase)
          end
        else
          Zoisite.logger.level = ActiveSupport::Logger.const_get(config.log_level.to_s.upcase)
          Zoisite.logger = ActiveSupport::BroadcastLogger.new(Zoisite.logger)
        end
      end

      initializer :initialize_error_reporter, group: :all do
        if config.consider_all_requests_local
          Zoisite.error.debug_mode = true
        else
          Zoisite.error.logger = Zoisite.logger
        end
      end

      initializer :initialize_event_reporter, group: :all do
        Zoisite.event.raise_on_error = config.consider_all_requests_local
        Zoisite.event.debug_mode = Zoisite.env.development?
      end
      ...
      initializer :initialize_cache, group: :all do
        ...
        unless Zoisite.cache
          Zoisite.cache = ActiveSupport::Cache.lookup_store(*config.cache_store)

          if Zoisite.cache.respond_to?(:middleware)
            config.middleware.insert_before(::Rack::Runtime, Zoisite.cache.middleware)
          end
        end
      end
