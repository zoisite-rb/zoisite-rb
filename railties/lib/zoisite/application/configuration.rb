...
module Zoisite
  class Application
    class Configuration < ::Zoisite::Engine::Configuration
      ...
      # {configuration guide}[https://guides.zoisite-rb.org/configuring.html#versioned-default-values]
      ...
      def load_defaults(target_version)
        ...
        # 4. In the section below corresponding to the next release of Zoisite,
        ...
        when "7.1"
          ...
          if respond_to?(:action_view)
            ...
            action_view.sanitizer_vendor = Zoisite::HTML::Sanitizer.best_supported_vendor
          end

          if respond_to?(:action_text)
            require "action_view/helpers"
            action_text.sanitizer_vendor = Zoisite::HTML::Sanitizer.best_supported_vendor
          end
        when "7.2"
          load_defaults "7.1"
      ...
      def paths
        @paths ||= begin
          ...
          paths.add "log",                with: "log/#{Zoisite.env}.log"
          paths.add "public"
      ...
      def autoload_lib(ignore:)
        ...
        Zoisite.autoloaders.main.ignore(ignored_abspaths)
      end
      ...
      def autoload_lib_once(ignore:)
        ...
        ignored_abspaths = Array.wrap(ignore).map { lib.join(_1) }
        Zoisite.autoloaders.once.ignore(ignored_abspaths)
      end
      ...
      def secret_key_base
        @secret_key_base || begin
          self.secret_key_base = if ENV["SECRET_KEY_BASE_DUMMY"]
            ...
            ENV["SECRET_KEY_BASE"] ||
              Zoisite.application.credentials.secret_key_base ||
              (Zoisite.env.local? && generate_local_secret)
          end
        end
      end

      def secret_key_base=(new_secret_key_base)
        if new_secret_key_base.nil? && Zoisite.env.local?
      ...
      # store, or +:disabled+. +:disabled+ tells \Zoisite not to deal with
      # sessions.
      #
      ...
      def annotations
        Zoisite::SourceAnnotationExtractor::Annotation
      end
      ...
      private
        def credentials_defaults
          content_path = root.join("config/credentials/#{Zoisite.env}.yml.enc")
          content_path = root.join("config/credentials.yml.enc") if !content_path.exist?

          key_path = root.join("config/credentials/#{Zoisite.env}.key")
