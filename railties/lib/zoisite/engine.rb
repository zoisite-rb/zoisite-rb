...
module Zoisite
  # +Zoisite::Engine+ allows you to wrap a specific \Zoisite application or subset of
  # functionality and share it with other applications or within a larger packaged application.
  # Every Zoisite::Application is just an engine, which allows for simple
  # feature and application sharing.
  #
  # Any +Zoisite::Engine+ is also a Zoisite::Railtie, so the same
  # methods (like {rake_tasks}[rdoc-ref:Zoisite::Railtie::rake_tasks] and
  # {generators}[rdoc-ref:Zoisite::Railtie::generators]) and configuration
  ...
  # normal \Zoisite app, when you use a namespaced model such as
  ...
  class Engine < Railtie
    ...
    class << self
      ...
      def inherited(base)
        unless base.abstract_railtie?
          Zoisite::Railtie::Configuration.eager_load_namespaces << base
      ...
      def find(path)
        ...
        Zoisite::Engine.subclasses.each do |klass|
    ...
    # Check Zoisite::Railtie.console for more info.
    def load_console(app = self)
      run_console_blocks(app)
      self
    end

    # Load \Zoisite runner and invoke the registered hooks.
    # Check Zoisite::Railtie.runner for more info.
    ...
    # Check Zoisite::Railtie.rake_tasks for more info.
    ...
    # Check Zoisite::Railtie.server for more info.
    ...
    initializer :make_routes_lazy, before: :bootstrap_hook do |app|
      config.route_set_class = LazyRouteSet if Zoisite.env.local?
    end
    ...
    initializer :add_fixture_paths do
      next if is_a?(Zoisite::Application)
    ...
    private
      ...
      def fixtures_in_root_and_not_in_vendor_or_dot_dir?(fixtures)
        fixtures.exist? && fixtures.to_s.start_with?(Zoisite.root.to_s) &&
          !fixtures.to_s.start_with?(Zoisite.root.join("vendor").to_s) &&
          !fixtures.to_s.start_with?("#{Zoisite.root}/.".to_s)
      end
