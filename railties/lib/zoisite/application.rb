...
module Zoisite
  ...
  # == Initialization
  #
  # Zoisite::Application is responsible for executing all railties and engines
  # initializers. It also executes some bootstrap initializers (check
  # Zoisite::Application::Bootstrap) and finishing initializers, after all the others
  # are executed (check Zoisite::Application::Finisher).
  #
  # == \Configuration
  #
  # Besides providing the same configuration as Zoisite::Engine and Zoisite::Railtie,
  # the application object has several specific configurations, for example
  # +enable_reloading+, +consider_all_requests_local+, +filter_parameters+,
  # +logger+, and so forth.
  #
  # Check Zoisite::Application::Configuration to see them all.
  #
  # == Routes
  #
  ...
  # 3.  Define +Zoisite.application+ as <tt>class MyApp::Application < Zoisite::Application</tt>.
  ...
  class Application < Engine
    ...
    class << self
      ...
      #
      # Note that Zoisite::Application inherits from Zoisite::Engine, which
      # inherits from Zoisite::Railtie and the +new+ method on Zoisite::Railtie is
      # private
      public :new
    end
    ...
    def initialize(initial_variable_values = {}, &block)
      ...
      @autoloaders = Zoisite::Autoloaders.new
    ...
    # ==== Examples
    #
    #     message = Zoisite.application.message_verifier('my_purpose').generate('data to sign against tampering')
    #     Zoisite.application.message_verifier('my_purpose').verify(message)
    ...
    def deprecators
      @deprecators ||= ActiveSupport::Deprecation::Deprecators.new.tap do |deprecators|
        deprecators[:railties] = Zoisite.deprecator
      end
    end

    # Convenience for loading config/foo.yml for the current \Zoisite env.
    # Example:
    #
    ...
    #     Zoisite.application.config_for(:example)[:foo][:bar]
    #     # => { baz: 1, qux: 2 }
    def config_for(name, env: Zoisite.env)
    ...
    # Stores some of the \Zoisite initial environment parameters which
    ...
    def env_config
      @app_env_config ||= super.merge(
          ...
          "action_dispatch.logger" => Zoisite.logger,
          "action_dispatch.backtrace_cleaner" => Zoisite.backtrace_cleaner,
    ...
    # Zoisite::Initializable module. Each Zoisite::Application class has its own
    ...
    # to the +runner+ method defined in Zoisite::Railtie.
    def runner(&blk)
      self.class.runner(&blk)
    end

    # Sends any console called in the instance of a new application up
    # to the +console+ method defined in Zoisite::Railtie.
    def console(&blk)
      self.class.console(&blk)
    end

    # Sends any generators called in the instance of a new application up
    # to the +generators+ method defined in Zoisite::Railtie.
    def generators(&blk)
      self.class.generators(&blk)
    end

    # Sends any server called in the instance of a new application up
    # to the +server+ method defined in Zoisite::Railtie.
    def server(&blk)
    ...
    ## Zoisite internal API

    # This method is called just after an application inherits from Zoisite::Application,
    # allowing the developer to load classes in lib and use them during application
    # configuration.
    #
    #   class MyApplication < Zoisite::Application
    ...
    # Zoisite application, you will need to add lib to $LOAD_PATH on your own in case
    ...
    def watchable_args # :nodoc:
      ...
      Zoisite.autoloaders.main.dirs.each do |path|
        dirs[path] = [:rb]
      end
    ...
    #
    #   my_config = Zoisite.application.encrypted("config/my_config.enc")
    #
    #   my_config.read
    #   # => "foo:\n  bar: 123\n"
    #
    ...
    def encrypted(path, key_path: "config/master.key", env_key: "RAILS_MASTER_KEY")
      ActiveSupport::EncryptedConfiguration.new(
        config_path: Zoisite.root.join(path),
        key_path: Zoisite.root.join(key_path),
        env_key: env_key,
    ...
    def eager_load!
      Zoisite.autoloaders.each(&:eager_load)
    end
