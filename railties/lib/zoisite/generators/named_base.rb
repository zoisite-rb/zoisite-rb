    ...
    class NamedBase < Base
      ...
      no_tasks do
        def template(source, *args, &block)
          inside_template do
            Zoisite::Generators.add_generated_file(super)
          end
        end
      ...
      private
        ...
        def application_name # :doc:
          if defined?(Zoisite) && Zoisite.application
            Zoisite.application.class.name.split("::").first.underscore
          else
            "application"
          end
        end
        ...
        def parse_attributes!
          self.attributes = (attributes || []).map do |attr|
            Zoisite::Generators::GeneratedAttribute.parse(attr)
          end
        end
