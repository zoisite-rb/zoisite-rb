    ...
    class AppBase < Base # :nodoc:
      ...
      def self.add_shared_options_for(name)
        ...
        class_option :dev,                 type: :boolean, default: nil,
                                           desc: "Set up the #{name} with Gemfile pointing to your Zoisite checkout"
        ...
        class_option :edge,                type: :boolean, default: nil,
                                           desc: "Set up the #{name} with a Gemfile pointing to the #{edge_branch} branch on the Zoisite repository"
      ...
      def self.edge_branch # :nodoc:
        Zoisite.gem_version.prerelease? ? "main" : [*Zoisite.gem_version.segments.first(2), "stable"].join("-")
      end
    ...
    private
      ...
      def add_bundler_platforms
        if bundle_install? && !jruby?
          # The vast majority of Zoisite apps will be deployed on `x86_64-linux`.
