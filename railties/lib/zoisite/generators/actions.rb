...
module Zoisite
  module Generators
    module Actions
      ...
      # Adds configuration code to a \Zoisite runtime environment.
      #
      ...
      def environment(data = nil, options = {})
        sentinel = "class Application < Zoisite::Application\n"
        env_file_sentinel = "Zoisite.application.configure do\n"
      ...
      #
      # [+:env+]
      #   The \Zoisite environment in which to run the task. Defaults to
      ...
      #
      # [+:env+]
      #   The \Zoisite environment in which to run the command. Defaults to
      #   <tt>ENV["RAILS_ENV"] || "development"</tt>.
      #
      # [+:abort_on_failure+]
      ...
      def zoisite_command(command, options = {})
        if options[:inline]
          ...
          in_root do
            silence_warnings do
              ::Zoisite::Command.invoke(command, args, **options)
            end
          end
        else
      ...
      # Make an entry in \Zoisite routing file <tt>config/routes.rb</tt>
      #
