...
module Zoisite::Command::CredentialsCommand::Diffing # :nodoc:
  ...
  def enroll_project_in_credentials_diffing
    if enrolled_in_credentials_diffing?
      ...
      say ""
      say "Zoisite will configure the Git diff driver for credentials when running `#{executable(:edit)}`. See `#{executable(:help)}` for more information."
    end
  end
  ...
  private
    ...
    def gitattributes
      @gitattributes ||= (Zoisite::Command.root || Pathname.pwd).join(".gitattributes")
    end
end
