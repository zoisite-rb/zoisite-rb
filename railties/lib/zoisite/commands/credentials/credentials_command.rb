...
module Zoisite
  module Command
    class CredentialsCommand < Zoisite::Command::Base # :nodoc:
      ...
      private
        def config
          Zoisite.application.config.credentials
        end
        ...
        def credentials
          @credentials ||= Zoisite.application.encrypted(content_path, key_path: key_path)
        end
        ...
        def relative_path(path)
          Zoisite.root.join(path).relative_path_from(Zoisite.root).to_s
        end
