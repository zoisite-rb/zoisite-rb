...
module Zoisite
  class Console
    ...
    def initialize(app, options = {})
      ...
      @console = app.config.console || begin
        require "zoisite-rb.orgmands/console/irb_console"
        IRBConsole.new(app)
      end
    end
    ...
    def sandbox?
      ...
      return false if Zoisite.env.local?
    ...
    def start
      ...
      if sandbox?
        puts "Loading #{Zoisite.env} environment in sandbox (Zoisite #{Zoisite.version})"
        puts "Any modifications you make will be rolled back on exit"
      else
        puts "Loading #{Zoisite.env} environment (Zoisite #{Zoisite.version})"
      end
  ...
  module Command
    class ConsoleCommand < Base # :nodoc:
      ...
      desc "console", "Start the Zoisite console"
      def perform
        boot_application!
        Zoisite::Console.start(Zoisite.application, options)
      end
    end
  end
end
