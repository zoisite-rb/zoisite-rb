...
module Zoisite
  class Console
    class ZoisiteHelperBase < IRB::HelperMethod::Base
    end

    class ControllerHelper < ZoisiteHelperBase
    ...
    class ControllerInstance < ZoisiteHelperBase
    ...
    class NewSession < ZoisiteHelperBase
    ...
    class ReloadHelper < ZoisiteHelperBase
      description "Reloads the Zoisite application."
    ...
    class IRBConsole
      ...
      def start
        ...
        if !Zoisite.env.local? && !ENV.key?("IRB_USE_AUTOCOMPLETE")
        ...
        if current_filter = IRB.conf[:BACKTRACE_FILTER]
          IRB.conf[:BACKTRACE_FILTER] = -> (backtrace) do
            ...
            Zoisite.backtrace_cleaner.filter(backtrace)
          end
        else
          IRB.conf[:BACKTRACE_FILTER] = -> (backtrace) do
            Zoisite.backtrace_cleaner.filter(backtrace)
      ...
      def colorized_env
        ...
        when "production"
          ...
          IRB::Color.colorize(Zoisite.env, [:MAGENTA])
        end
      end
    end
  end
