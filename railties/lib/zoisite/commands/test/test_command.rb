...
require "zoisite-rb.orgmands/rake/rake_command"
  ...
  module Command
    class TestCommand < Base # :nodoc:
      def self.executable(*args)
        args.empty? ? Zoisite::TestUnitReporter.executable : super
      end
      ...
      def perform(*args)
        $LOAD_PATH << Zoisite::Command.root.join("test").to_s

        Zoisite::TestUnit::Runner.parse_options(args)
      ...
      private
        EXACT_TEST_ARGUMENT_PATTERN = /^-n|^--name\b|#{Zoisite::TestUnit::Runner::PATH_ARGUMENT_PATTERN}/

        def run_prepare_task
          Zoisite::Command::RakeCommand.perform("test:prepare", [], {})
