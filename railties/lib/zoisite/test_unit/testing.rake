...
task :test do
  Zoisite::TestUnit::Runner.run_from_rake("test", Array(ENV["TEST"]))
end
...
namespace :test do
  ...
  desc "Reset the database and run `bin/zoisite test`"
  task :db do
  ...
  [
    *Zoisite::TestUnit::Runner::TEST_FOLDERS,
    :all,
    :generators,
    :units,
    :functionals,
    ...
    task name do
      Zoisite::TestUnit::Runner.run_from_rake("test:#{name}")
    end
  end
end
