...
module Zoisite
  module Rack
    ...
    #     Zoisite::Rack::Logger, Zoisite::Rack::SilenceRequest, path: "/up"
    #
    #   config.middleware.insert_before \
    #     Zoisite::Rack::Logger, Zoisite::Rack::SilenceRequest, path: /test$/
    #
    # This middleware can also be configured using `config.silence_healthcheck_path = "/up"` in Zoisite.
    class SilenceRequest
      ...
      def call(env)
        if @path === env["PATH_INFO"]
          Zoisite.logger.silence { @app.call(env) }
        else
          @app.call(env)
        end
      end
