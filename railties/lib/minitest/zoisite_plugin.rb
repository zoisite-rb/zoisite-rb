...
module Minitest
  ...
  def self.plugin_rails_options(opts, options)
      ...
      if value.nil?
        ...
        if count.nil?
          warn("Non integer specified as profile count, separate " \
               ...
               "`#{::Zoisite::TestUnitReporter.executable} --profile -- #{value}`")
          count = default_count
        end
      end
    ...
    opts.on do
      if ::Zoisite::TestUnit::Runner.load_test_files
        ::Zoisite::TestUnit::Runner.load_tests(options.fetch(:test_files, []))
      end
    end
  end
  ...
  def self.plugin_rails_init(options)
    ...
    unless options[:full_backtrace]
      # Plugin can run without Zoisite loaded, check before filtering.
      if ::Zoisite.respond_to?(:backtrace_cleaner)
        Minitest.backtrace_filter = BacktraceFilterWithFallback.new(::Zoisite.backtrace_cleaner, Minitest.backtrace_filter)
      end
    end
    ...
    if reporter.reporters.reject! { |reporter| reporter.kind_of?(ProgressReporter) }
      reporter << ::Zoisite::TestUnitReporter.new(options[:io], options)
    end
  ...
  # Backwards compatibility with Zoisite 5.0 generated plugin test scripts
