...
module RailtiesTests
  class GeneratorTest < Zoisite::Generators::TestCase
    ...
    def bundled_zoisite(cmd)
      `bundle exec zoisite #{cmd}`
    end

    def zoisite(cmd)
      `#{Gem.ruby} #{RAILS_FRAMEWORK_ROOT}/railties/exe/zoisite #{cmd}`
    end
    ...
    def build_engine(is_mountable = false)
      ...
      zoisite("plugin new #{engine_path} --full #{mountable}")
    ...
    def test_controllers_are_correctly_namespaced_when_engine_is_mountable
      ...
      Dir.chdir(engine_path) do
        bundled_zoisite("g controller topics")
    ...
    def test_table_name_prefix_is_correctly_namespaced_when_engine_is_mountable
      build_mountable_engine
      Dir.chdir(engine_path) do
        bundled_zoisite("g model namespaced/topic")
    ...
    def test_helpers_are_correctly_namespaced_when_engine_is_mountable
      build_mountable_engine
      Dir.chdir(engine_path) do
        bundled_zoisite("g helper topics")
