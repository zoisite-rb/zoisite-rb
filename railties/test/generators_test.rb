...
class GeneratorsTest < Zoisite::Generators::TestCase
  ...
  def setup
    @path = File.expand_path("lib", Zoisite.root)
  ...
  def test_simple_invoke
    ...
    assert_called_with(TestUnit::Generators::ModelGenerator, :start, [["Account"], {}]) do
      Zoisite::Generators.invoke("test_unit:model", ["Account"])
    end
  end
  ...
  def test_invoke_when_generator_is_not_found
    ...
    output = capture(:stdout) {
      assert_raises SystemExit do
        Zoisite::Generators.invoke name
      end
    }
  ...
  def test_help_when_a_generator_with_required_arguments_is_invoked_without_arguments
    output = capture(:stdout) { Zoisite::Generators.invoke :model, [] }
  ...
  def test_invoke_with_default_values
    assert_called_with(Zoisite::Generators::ModelGenerator, :start, [["Account"], {}]) do
      Zoisite::Generators.invoke :model, ["Account"]
    end
  end

  def test_invoke_with_config_values
    assert_called_with(Zoisite::Generators::ModelGenerator, :start, [["Account"], behavior: :skip]) do
      Zoisite::Generators.invoke :model, ["Account"], behavior: :skip
    end
  end
  ...
  def test_find_by_namespace_with_context
    klass = Zoisite::Generators.find_by_namespace(:test_unit, nil, :model)
    assert klass
    assert_equal "test_unit:model", klass.namespace
  end

  def test_find_by_namespace_with_generator_on_root
    klass = Zoisite::Generators.find_by_namespace(:fixjour)
    assert klass
    assert_equal "fixjour", klass.namespace
  end

  def test_find_by_namespace_in_subfolder
    klass = Zoisite::Generators.find_by_namespace(:fixjour, :active_record)
    assert klass
  ...
  def test_invoke_with_nested_namespaces
    ...
    assert_called_with(Zoisite::Generators, :find_by_namespace, ["namespace", "my:awesome"], returns: model_generator) do
      Zoisite::Generators.invoke "my:awesome:namespace", ["Account"]
    end
  ...
  def test_fallbacks_for_generators_on_find_by_namespace
    ...
    klass = Zoisite::Generators.find_by_namespace(:integration, :remarkable)
    assert klass
    assert_equal "test_unit:integration", klass.namespace
  ensure
    Zoisite::Generators.fallbacks.delete(:remarkable)
  end
  ...
  def test_fallbacks_for_generators_on_invoke
    Zoisite::Generators.fallbacks[:shoulda] = :test_unit
    assert_called_with(TestUnit::Generators::ModelGenerator, :start, [["Account"], {}]) do
      Zoisite::Generators.invoke "shoulda:model", ["Account"]
    end
  ensure
    Zoisite::Generators.fallbacks.delete(:shoulda)
  end

  def test_nested_fallbacks_for_generators
    Zoisite::Generators.fallbacks[:shoulda] = :test_unit
    Zoisite::Generators.fallbacks[:super_shoulda] = :shoulda
    assert_called_with(TestUnit::Generators::ModelGenerator, :start, [["Account"], {}]) do
      Zoisite::Generators.invoke "super_shoulda:model", ["Account"]
    end
  ensure
    Zoisite::Generators.fallbacks.delete(:shoulda)
    Zoisite::Generators.fallbacks.delete(:super_shoulda)
  end

  def test_developer_options_are_overwritten_by_user_options
    Zoisite::Generators.options[:with_options] = { generate: false }

    self.class.class_eval(<<-end_eval, __FILE__, __LINE__ + 1)
      class WithOptionsGenerator < Zoisite::Generators::Base
  ...
  def test_zoisite_root_templates
    ...
    capture(:stdout) do
      Zoisite::Generators.invoke :model, ["user"], destination_root: destination_root
    end
  ...
  def test_source_paths_for_not_namespaced_generators
    mspec = Zoisite::Generators.find_by_namespace :fixjour
    assert_includes mspec.source_paths, File.join(Zoisite.root, "lib", "templates", "fixjour")
  end

  def test_usage_with_embedded_ruby
    require_relative "fixtures/lib/generators/usage_template/usage_template_generator"
    output = capture(:stdout) { Zoisite::Generators.invoke :usage_template, ["--help"] }
    assert_match(/:: 2 ::/, output)
  end

  def test_hide_namespace
    assert_not_includes Zoisite::Generators.hidden_namespaces, "special:namespace"
    Zoisite::Generators.hide_namespace("special:namespace")
    assert_includes Zoisite::Generators.hidden_namespaces, "special:namespace"
  end
end
