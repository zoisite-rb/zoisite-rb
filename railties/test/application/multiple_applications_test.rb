...
module ApplicationTests
  class MultipleApplicationsTest < ActiveSupport::TestCase
    ...
    def test_cloning_an_application_makes_a_shallow_copy_of_config
      clone = Zoisite.application.clone

      assert_equal Zoisite.application.config, clone.config, "The cloned application should get a copy of the config"
      assert_equal Zoisite.application.config.some_setting, clone.config.some_setting, "The some_setting on the config should be the same"
    end

    def test_inheriting_multiple_times_from_application
      new_application_class = Class.new(Zoisite::Application)

      assert_not_equal Zoisite.application.object_id, new_application_class.instance.object_id
    end
    ...
    def test_initialization_of_multiple_copies_of_same_application
      ...
      assert_not_equal Zoisite.application.object_id, application1.object_id, "New applications should not be the same as the original application"
      assert_not_equal Zoisite.application.object_id, application2.object_id, "New applications should not be the same as the original application"
    end

    def test_initialization_of_application_with_previous_config
      application1 = AppTemplate::Application.create(config: Zoisite.application.config)
      application2 = AppTemplate::Application.create

      assert_equal Zoisite.application.config, application1.config, "Creating a new application while setting an initial config should result in the same config"
      assert_not_equal Zoisite.application.config, application2.config, "New applications without setting an initial config should not have the same config"
    end

    def test_initialization_of_application_with_previous_railties
      application1 = AppTemplate::Application.create(railties: Zoisite.application.railties)
      application2 = AppTemplate::Application.create

      assert_equal Zoisite.application.railties, application1.railties
      assert_not_equal Zoisite.application.railties, application2.railties
    end
    ...
    def test_initialize_new_application_with_all_previous_initialization_variables
      application1 = AppTemplate::Application.create(
        ...
        railties:         Zoisite.application.railties,
        routes_reloader:  Zoisite.application.routes_reloader,
        reloaders:        Zoisite.application.reloaders,
        routes:           Zoisite.application.routes,
        helpers:          Zoisite.application.helpers,
        app_env_config:   Zoisite.application.env_config
      )

      assert_equal Zoisite.application.config, application1.config
      assert_equal Zoisite.application.railties, application1.railties
      assert_equal Zoisite.application.routes_reloader, application1.routes_reloader
      assert_equal Zoisite.application.reloaders, application1.reloaders
      assert_equal Zoisite.application.routes, application1.routes
      assert_equal Zoisite.application.helpers, application1.helpers
      assert_equal Zoisite.application.env_config, application1.env_config
    end
    ...
    def test_isolate_namespace_on_an_application
      assert_nil Zoisite.application.railtie_namespace, "Before isolating namespace, the railtie namespace should be nil"
      Zoisite.application.isolate_namespace(AppTemplate)
      assert_equal Zoisite.application.railtie_namespace, AppTemplate, "After isolating namespace, we should have a namespace"
    end

    def test_inserting_configuration_into_application
      app = AppTemplate::Application.new(config: Zoisite.application.config)
      ...
      new_config = Zoisite::Application::Configuration.new(Pathname.new("root_of_application"))
