...
class MailerGeneratorTest < Zoisite::Generators::TestCase
  ...
  def test_invokes_default_test_framework
    ...
    assert_file "test/mailers/previews/notifier_mailer_preview.rb" do |preview|
      assert_match(/\# Preview all emails at http:\/\/localhost:3000\/zoisite\/mailers\/notifier_mailer/, preview)
      assert_match(/class NotifierMailerPreview < ActionMailer::Preview/, preview)
      assert_match(/\# Preview this email at http:\/\/localhost:3000\/zoisite\/mailers\/notifier_mailer\/foo/, preview)
      assert_instance_method :foo, preview do |foo|
        assert_match(/NotifierMailer\.foo/, foo)
      end
      assert_match(/\# Preview this email at http:\/\/localhost:3000\/zoisite\/mailers\/notifier_mailer\/bar/, preview)
  ...
  def test_mailer_with_namespaced_mailer
    ...
    assert_file "test/mailers/previews/farm/animal_mailer_preview.rb" do |preview|
      assert_match(/\# Preview all emails at http:\/\/localhost:3000\/zoisite\/mailers\/farm\/animal_mailer/, preview)
      assert_match(/class Farm::AnimalMailerPreview < ActionMailer::Preview/, preview)
      assert_match(/\# Preview this email at http:\/\/localhost:3000\/zoisite\/mailers\/farm\/animal_mailer\/moos/, preview)
    end
