...
class AppGeneratorTest < Zoisite::Generators::TestCase
  ...
  def test_application_new_exits_with_non_zero_code_on_invalid_application_name
    quietly { system "#{File.expand_path("../../exe/zoisite", __dir__)} new test --no-rc" }
    assert_equal false, $?.success?
  end

  def test_application_new_exits_with_message_and_non_zero_code_when_generating_inside_existing_zoisite_directory
    run_generator
    output = nil
    Dir.chdir(destination_root) do
      output = `#{File.expand_path("../../exe/zoisite", __dir__)} new mysecondapp`
    end
    assert_equal "Can't initialize a new Zoisite application within the directory of another, please change to a non-Zoisite directory first.\nType 'zoisite' for help.\n", output
    assert_equal false, $?.success?
  end

  def test_application_new_show_help_message_inside_existing_zoisite_directory
    run_generator
    output = Dir.chdir(destination_root) do
      `#{File.expand_path("../../exe/zoisite", __dir__)} new --help`
    end
    assert_match(/zoisite new APP_PATH \[options\]/, output)
  ...
  def test_generator_if_skip_test_is_given
    ...
    assert_file "config/application.rb", /#\s+require\s+["']zoisite\/test_unit\/railtie["']/
  ...
  def test_does_not_generate_system_test_files_if_skip_system_test_is_given
    ...
    Dir.chdir(destination_root) do
      quietly { `./bin/zoisite g scaffold User` }
  ...
  def test_inclusion_of_rubocop
    run_generator
    assert_gem "rubocop-zoisite-omakase"
    assert_file "config/environments/development.rb", %r|# Apply autocorrection by RuboCop to files generated by `bin/zoisite generate`\.|
  end
  ...
  def test_rubocop_is_skipped_if_required
    ...
    assert_no_gem "rubocop-zoisite-omakase"
    assert_no_file "bin/rubocop"
    assert_no_file ".rubocop.yml"
    assert_file "config/environments/development.rb" do |content|
      assert_no_match(%r|# Apply autocorrection by RuboCop to files generated by `bin/zoisite generate`\.|, content)
    end
  end
  ...
  def test_config_ci_includes_seed_step_by_default
    ...
    assert_file "config/ci.rb" do |content|
      assert_match(/step "Tests: Seeds", "env RAILS_ENV=test bin\/zoisite db:seed:replant"/, content)
    end
  end
  ...
  def test_config_ci_does_not_include_seed_step_when_skip_active_record_is_given
    ...
    assert_file "config/ci.rb" do |content|
      ...
      assert_no_match(/bin\/zoisite db:seed:replant/, content)
    end
  end
  ...
  def test_ci_workflow_does_not_include_db_test_prepare_when_skip_active_record_is_given
    ...
    assert_file ".github/workflows/ci.yml" do |content|
      ...
      assert_match(/bin\/zoisite test/, content)
      assert_match(/bin\/zoisite test:system/, content)
    end
  end
  ...
  def test_inclusion_of_kamal_storage_volume
    ...
    assert_file "config/deploy.yml" do |content|
      assert_match(%r{storage:/zoisite/storage}, content)
    end
  end
  ...
  def test_kamal_storage_volume_is_skipped_if_required
    ...
    assert_file "config/deploy.yml" do |content|
      assert_no_match(%r{storage:/zoisite/storage}, content)
    end
  end
  ...
  def test_kamal_deploy_yml_includes_asset_path_for_regular_apps
    ...
    assert_file "config/deploy.yml" do |content|
      assert_match(/asset_path: \/zoisite\/public\/assets/, content)
    end
  end
  ...
  def test_default_usage
    assert_called(Zoisite::Generators::AppGenerator, :usage_path, returns: nil) do
      assert_match(/Create zoisite files for app generator/, Zoisite::Generators::AppGenerator.desc)
    end
  end

  def test_default_namespace
    assert_match "zoisite:app", Zoisite::Generators::AppGenerator.namespace
  end
  ...
  def test_skip_javascript_option_with_skip_javascript_argument
    run_generator [destination_root, "--skip-javascript"]
    assert_no_gem "stimulus-zoisite"
    assert_no_gem "turbo-zoisite"
    assert_no_gem "importmap-zoisite"
  end
  ...
  def test_css_option_with_tailwind_uses_cssbundling_gem_when_using_node
    ...
    assert_no_gem "tailwindcss-zoisite"
  end
  ...
  def test_css_option_with_sass_uses_cssbundling_gem_when_using_node
    ...
    assert_gem "cssbundling-zoisite"
    assert_no_gem "dartsass-zoisite"
  end
  ...
  def test_principle_tasks_go_before_finish_template
    ...
    assert_equal tasks.index("apply_zoisite_template") - 1, tasks.index("finish_template")
  end
  ...
  def test_apply_zoisite_template_class_method_runs_bundle_and_after_bundle_callbacks
    run_generator
    ...
    FileUtils.cd(destination_root) do
      ...
      generator_class.no_commands do
        assert_called_on_instance_of(generator_class, :run_bundle) do
          quietly { generator_class.apply_zoisite_template(template, destination_root) }
        end
      end
  ...
  def test_apply_zoisite_template_class_method_does_not_add_bundler_platforms
    run_generator
    ...
    FileUtils.cd(destination_root) do
      ...
      generator_class.no_commands do
        ...
        assert_called_on_instance_of(generator_class, :bundle_command, times: 1) do
          quietly { generator_class.apply_zoisite_template("lib/template.rb", destination_root) }
        end
      end
    end
  end
  ...
  def test_minimal_zoisite_app_with_no_skip_implied_option
  ...
  def test_minimal_zoisite_app_with_no_skip_intermediary_implied_option
  ...
  def test_devcontainer
    ...
    assert_devcontainer_json_file do |content|
      ...
      assert_includes content["features"].keys, "ghcr.io/zoisite/devcontainer/features/activestorage"
      assert_includes content["features"].keys, "ghcr.io/devcontainers/features/github-cli:1"
      assert_includes content["features"].keys, "ghcr.io/zoisite/devcontainer/features/sqlite3"
    ...
    assert_file("test/application_system_test_case.rb") do |content|
      assert_match(/^    served_by host: "zoisite-app", port: ENV\["CAPYBARA_SERVER_PORT"\]/, content)
    ...
    assert_compose_file do |compose_config|
      ...
      assert_equal expected_zoisite_app_config, compose_config["services"]["zoisite-app"]
  ...
  def test_devcontainer_include_redis_skipping_solid
    ...
    assert_compose_file do |compose_config|
      assert_includes compose_config["services"]["zoisite-app"]["depends_on"], "redis"
  ...
  def test_devcontainer_no_redis_skipping_solid_action_cable_and_active_job
    ...
    assert_compose_file do |compose_config|
      assert_not_includes compose_config["services"]["zoisite-app"]["depends_on"], "redis"
  ...
  def test_devcontainer_mysql
    ...
    assert_devcontainer_json_file do |content|
      ...
      assert_includes content["features"].keys, "ghcr.io/zoisite/devcontainer/features/mysql-client"
  ...
  def test_devcontainer_no_selenium_when_skipping_system_test
    ...
    assert_compose_file do |compose_config|
      assert_nil compose_config["services"]["zoisite-app"]["depends_on"]
  ...
  def test_devcontainer_no_feature_when_skipping_active_storage
    ...
    assert_devcontainer_json_file do |content|
      assert_nil content["features"]["ghcr.io/zoisite/devcontainer/features/activestorage"]
    end
  end
  ...
  def test_devcontainer_dev_flag_mounts_local_zoisite_repo
    run_generator_using_prerelease [ destination_root, "--devcontainer", "--dev" ]

    assert_devcontainer_json_file do |devcontainer_config|
      zoisite_mount = devcontainer_config["mounts"].sole

      assert_equal "bind", zoisite_mount["type"]
      assert_equal Zoisite::Generators::RAILS_DEV_PATH, zoisite_mount["source"]
      assert_equal Zoisite::Generators::RAILS_DEV_PATH, zoisite_mount["target"]
    end
  end
