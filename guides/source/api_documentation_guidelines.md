...
This is due to differences between GitHub Markdown and the generated RDoc that is published at [api.zoisite-rb.org](https://api.zoisite-rb.org) and [edgeapi.zoisite-rb.org](https://edgeapi.zoisite-rb.org).

For example, we use `[link:classes/ActiveRecord/Base.html]` to create a link to the `ActiveRecord::Base` class generated by RDoc.

This is preferred over absolute URLs such as `[https://api.zoisite-rb.org/classes/ActiveRecord/Base.html]`, which would take the reader outside their current documentation version (e.g. edgeapi.zoisite-rb.org).
...
[#encrypt_and_sign]: https://api.zoisite-rb.org/classes/ActiveSupport/MessageEncryptor.html#method-i-encrypt_and_sign
