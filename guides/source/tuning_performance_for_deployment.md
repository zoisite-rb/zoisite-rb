...
This guide covers performance and concurrency configuration for deploying your production Zoisite application.
...
Puma is Zoisite' default application server and the most commonly used server across the community.
...
Generally speaking, a well-crafted Zoisite application that isn't suffering from slow SQL queries or N+1 problems doesn't
...
As for Zoisite 7.2, if your application is running on Ruby 3.3 or superior, YJIT will automatically be enabled by Zoisite
by default.
Older versions of Zoisite or Ruby have to enable it manually, please refer to the
...
The default Dockerfile generated by Zoisite already comes preconfigured to install and use `jemalloc`. But if your hosting
...
Because every Zoisite application is different, and that every Zoisite user may want to optimize for different properties,
...
You should not normally check the time in your Zoisite log file.
That time is only how long Zoisite took to process the request. It does not include time taken by the application server.
